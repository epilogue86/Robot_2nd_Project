            ...
            try:
                ...
                try:
                    ...
                    if data:
                        ...
                        cmd_process(self, self.received_data)
...
def keyboard_control_thread(node):
        while rclpy.ok():
            ...
            cmd_process(node, key)

        
def cmd_process(node, key):
    node.get_logger().info("cmd_process")
    if key in sectors:
        x, y, w = sectors[key]
...
def main():
    ...
    keyboard_thread = threading.Thread(target=keyboard_control_thread, args=(node,), daemon=True)
